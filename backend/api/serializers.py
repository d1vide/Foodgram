...
class RecipeUnsafeSerializer(serializers.ModelSerializer):
    ...
    ingredients = RecipeIngredientSerializer(many=True, source='recipeingredient', required=True)
    ...
    def update(self, recipe, validated_data):
        ...
        if recipeingredient_data:
            ...
            for ingredient in recipeingredient_data:
                RecipeIngredient.objects.get_or_create(ingredients=ingredient.get('id'),
                                                       recipes=recipe,
                                                       amount=ingredient.get('amount'))
...
class RecipeSafeSerializer(serializers.ModelSerializer):
    ...
    def get_ingredients(self, obj):
        recipe = obj
        ingredients = recipe.ingredients.annotate(amount=F('recipeingredient__amount')).values(
            'id',
            'name',
            'amount',
            'measurement_unit',
...
class SubscribeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('email', 'username', 'last_name', 'first_name', 'avatar')
    ...
    def get_recipes(self, obj):
        ...
        serializer = FavoriteAndShoppingCartResponseSerializer(recipes, many=True, read_only=True)
        return serializer.data
